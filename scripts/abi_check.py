...
class AbiChecker(object):
    ...
    def get_clean_worktree_for_git_revision(self, git_rev):
        ...
        git_worktree_path = os.path.join(self.report_dir, git_rev)
...
def run_main():
    try:
        ...
        parser.add_argument(
            "-r", "--report_dir", type=str, default="tmp",
            help="directory where reports are stored, default is tmp",
        )
        parser.add_argument(
        ...
        abi_check = AbiChecker(
            abi_args.repo_path, abi_args.report_dir,
            abi_args.old_rev, abi_args.new_rev,
            abi_args.keep_all_reports
        )
