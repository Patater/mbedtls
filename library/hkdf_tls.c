                   ...
                   unsigned char *dstbuf, int buflen )
{
    ...
    {
        mbedtls_printf(
            "Derive_Secret: Incorrect length of hash - mlen (%d) != L (%d)\n",
            mlen, L );
    ...
    ret = hkdfExpandLabel( hash_alg, secret, slen, label, llen, hashValue, L, L,
                           dstbuf, buflen );
 ...
 *
 * [sender] denotes the sending side and the Secret value is provided by the
 * function caller. We generate server and client side keys in a single function
 * call.
 */
int makeTrafficKeys( mbedtls_md_type_t hash_alg,
                     const unsigned char *client_key,
                     const unsigned char *server_key, int slen, int keyLen,
                     int ivLen, KeySet *keys )
{
    int ret = 0;
    ...
    {
        mbedtls_printf(
            "makeTrafficKeys(): Error allocating clientWriteKey.\n" );
        return( MBEDTLS_ERR_HKDF_BUFFER_TOO_SMALL );
    }

    ret = hkdfExpandLabel(
        hash_alg, client_key, slen, (const unsigned char *) "key", 3,
        (const unsigned char *) "", 0, keyLen, keys->clientWriteKey, keyLen );
    ...
    {
        mbedtls_printf(
            "makeTrafficKeys(): Error allocating serverWriteKey.\n" );
        return( ret );
    }

    ret = hkdfExpandLabel(
        hash_alg, server_key, slen, (const unsigned char *) "key", 3,
        (const unsigned char *) "", 0, keyLen, keys->serverWriteKey, keyLen );
    ...
    {
        mbedtls_printf(
            "makeTrafficKeys(): Error allocating clientWriteIV.\n" );
        return( ret );
    }

    ret = hkdfExpandLabel(
        hash_alg, client_key, slen, (const unsigned char *) "iv", 2,
        (const unsigned char *) "", 0, ivLen, keys->clientWriteIV, ivLen );
    ...
    {
        mbedtls_printf(
            "makeTrafficKeys(): Error allocating serverWriteIV.\n" );
        return( ret );
    }

    ret = hkdfExpandLabel(
        hash_alg, server_key, slen, (const unsigned char *) "iv", 2,
        (const unsigned char *) "", 0, ivLen, keys->serverWriteIV, ivLen );
                     ...
                     unsigned char *buf, int blen )
{
    ...
    {
        mbedtls_printf(
            "mbedtls_md_info_from_type() failed in hkdfExpandLabel()." );
        goto clean_up;
    }
...
int mbedtls_hkdf_tls_encode_label( const unsigned char *label, int llen,
