                                               ...
                                               size_t *olen )
{
    ...
    *olen = 0;
    ...
    for( md = ssl->conf->sig_hashes; *md != MBEDTLS_MD_NONE; md++ )
    {
        ...
        sig_alg_len += 2;
#endif
        if( sig_alg_len > MBEDTLS_SSL_MAX_SIG_HASH_ALGORITHMS )
        {
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "malformed sig-hash-alg extension" ) );
            return( MBEDTLS_ERR_SSL_BAD_CONFIG );
        }
    }
                               ...
                               size_t *olen )
{
    ...
    *olen = 0;
    ...
    /* Overestimate alpn entry size to avoid computational overhead */
    for( cur = ssl->conf->alpn_list; *cur != NULL; cur++ )
    {
        alpnlen += (unsigned char)( strlen( *cur ) & 0xFF ) + 1;

        if( alpnlen > MBEDTLS_SSL_MAX_ALPN )
        {
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "malformed alpn extension" ) );
            return( MBEDTLS_ERR_SSL_BAD_CONFIG );
        }
    }
...
static int ssl_write_client_hello( mbedtls_ssl_context *ssl )
{
     ...
     * consisting of the 38 bytes shown above, the session identifier and
     * its length, and potentially a DTLS cookie and its length.
     *
                                         ...
                                         size_t len )
{
...
#endif /* MBEDTLS_SSL_RENEGOTIATION */
    {
        if( len != 1 || buf[0] != 0x00 )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "non-zero length "
                                        "renegotiation info" ) );
...
static int ssl_parse_server_hello( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->in_msgtype != MBEDTLS_SSL_MSG_HANDSHAKE )
    {
        ...
        if( ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS )
        {
            ...
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "non-handshake message "
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "server version out of bounds -  min: "
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "server hello, bad compression: "
                                    "%d", comp ) );
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "ciphersuite info for "
                                    "%04x not found", i ) );
    ...
    while( ext_len )
    {
        ...
        case MBEDTLS_TLS_EXT_MAX_FRAGMENT_LENGTH:
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "found max_fragment_length "
                                        "extension" ) );
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        ...
        default:
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "unknown extension found: "
                                        "%d (ignoring)", ext_id ) );
        }
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "legacy renegotiation, "
                                    "breaking off handshake" ) );
        handshake_failure = 1;
    }
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "renegotiation_info extension "
                                    "missing (secure)" ) );
        handshake_failure = 1;
    }
    else if( ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS &&
        ...
        ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA )
    {
        ...
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
        {
            if( ssl_parse_signature_algorithm( ssl, &p, end,
                                               &md_alg, &pk_alg ) != 0 )
            {
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad server key "
                                            "exchange message" ) );
                return( MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE );
            }
...
static int ssl_parse_certificate_request( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
    {
        ...
        {
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "Supported Signature "
                                        "Algorithm found: %d,%d",
